...
export default defineNitroConfig({
  ...
  storage: {
    // Local/dev: filesystem under ./server/.data (the default).
    // On Vercel serverless the project dir is read-only, so we auto-detect
    // the VERCEL env var and fall back to /tmp/data which is always writable
    // (ephemeral but re-seeded on every cold start from the built-in SEED).
    // For persistent storage set NITRO_STORAGE_DRIVER + its credentials:
    ...
    data: {
      ...
      base:
        ...
        (process.env.VERCEL ? "/tmp/data" : "./server/.data"),
    },
  },
});
